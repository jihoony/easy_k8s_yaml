...
3. **설정 프리셋 저장 및 불러오기 (Preset Export/Import) with 파일 내용 보존**
   - 현재 입력된 모든 폼 입력 상태, 수동 볼륨 설정, 환경변수 및 체크박스 활성화 값들을 하나의 `.json` 파일로 내보낼 수 있습니다.
   - 특히 업로드한 **ConfigMap 및 Secret 파일들의 실제 컨텐츠 본문까지 Base64 데이터 스트링으로 직렬화하여 함께 백업**하므로, 프리셋을 가져오기 한 후 파일 재업로드 등의 중복 절차 없이 **즉시 드라이런 YAML 생성이 가능**합니다.

4. **인터랙티브 접기/펼치기 UI (기본 접힘)**
   - 폼 패널의 화면 공간을 극대화하기 위해 각 설정 섹션은 **기본적으로 접힌(Collapsed) 상태**로 로딩됩니다.
   - 각 헤더 영역을 클릭하면 아코디언처럼 부드러운 화살표 회전 애니메이션과 함께 입력 본문이 접히고 펼쳐집니다.

5. **3가지 테마 스타일 모드 (Theme Switcher)**
...
8. **비설치형 Kubectl 환경 (Docker Sidecar)**
...
2. **YAML Post-Processor**: `kubectl` 명령어 옵션만으로는 제어하기 힘든 복잡한 파드 상세 스펙(`volumeMounts[].subPath`, `valueFrom.secretKeyRef`, `valueFrom.configMapKeyRef`, Service의 `nodePort` 매핑 등)을 Go의 `yaml.v3` 파서를 이용하여 AST 기반으로 안전하게 조작(Injection)하여 완전한 리소스 YAML을 완성합니다. **이때 최종 직렬화 시 들여쓰기를 표준 2칸(`SetIndent(2)`)으로 강제 지정하여 복사나 배포에 적합한 가독성 높은 YAML 규격을 보장합니다.**
